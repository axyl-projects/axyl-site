---
title: AxylOS Cyber Hub
description: Running vulnerable labs and Cyber Security platforms.
---
import ImageComponent from "@components/ImageComponent.astro";
import ThemedImage from '@components/ThemedImage.astro';

[AxylOS Cyber Hub (ACH)](https://github.com/AxylOS-OS/AxylOS-cyber-hub) is a forked version of [Vanilla Control Center](https://github.com/Vanilla-OS/vanilla-control-center) with several differences in order to fit the needs of Cyber Security users on AxylOS.
...
## Usage
**AxylOS Cyber Hub** can be run by clicking its icon in the Application List or running `AxylOS-cyber-hub` in the terminal.
...
AxylOS Cyber Hub makes AxylOS to be the **Wazuh Management Server** to collect **centrally** all information about **threats** and **vulnerabilities** through all endpoints where you deployed Wazuh agents.

Wazuh Management Server can be initiated easily on AxylOS by starting its container in AxylOS Cyber Hub.
...
To monitor AxylOS itself, deploy the Wazuh agent as follows:
```shell
...
* What's the IP Address or hostname of the Wazuh server? **localhost**
  :::tip
  **localhost** because we are deploying the agent on AxylOS itself that works also as Wazuh Management Server (so as SIEM/XDR). If you are setting it to a different machine, type IP address of the target machine itself
  :::
...
Completed.
```
If Wazuh is still not started on AxylOS Cyber Hub, start it and wait for 2-3 minutes for its initialization. After that, open the browser and navigate to `https://localhost`. You should get Wazuh login page where to type the default access credentials `admin:SecretPassword` and, once landed on Wazuh dashboard, you should see the **Active agents** number to 1, that is the Wazuh agent just installed.

Unleash **AxylOS** to be your **Blue Team OS**! Monitor your OS and your infrastructure for vulnerabilities and threats in order to be always aware of your security posture!
...
id = "vulhub/activemq"
name = "ActiveHello"
path_docker = "/usr/share/AxylOS-platform-center/vulhub/activemq/CVE-2015-5254/docker-compose.yml"
path_readme = "/usr/share/AxylOS-platform-center/vulhub/activemq/CVE-2015-5254/README.md"
