...
If you wish to submit a new pentesting tool that is not in any Arch Linux or AxylOS repository, this section will help you.

Pentesting tools are mainly maintained in AxylOS repository, so your new pentesting tools must be stored there.
...
1. Create a **PKGBUILD** file containing the rules for compiling and installing `mytoolname`. According to the type of the tool, it can be useful to start by one of [BlackArch PKGBUILD templates](https://github.com/BlackArch/blackarch-pkgbuilds)
   ...
   * copy the content of the PKGBUILD template file inside a new file named `PKGBUILD` created in `AxylOS/packages/pentesting/<mytoolname>` folder.
2. Fill the variable inside PKGBUILD according to the tool information. If you are new on PKGBUILD, give a look to the [official Arch Linux documentation](https://wiki.archlinux.org/title/PKGBUILD). You can check several examples in [AxylOS repository](https://github.com/AxylOS-OS/AxylOS/tree/main/packages/pentesting)
   * when you write information inside PKGBUILD, there are several best practices you must follow:
      ...
      * In `groups=`, insert the right AxylOS categories
...
7. Go to your draft pull request and submit it. You need to wait for AxylOS maintainers that will check your submission and merge your pull request.
...
:::caution
If your submitted tool has some dependencies that are not in AxylOS or Arch Linux repositories, you need to create a PKGBUILD dedicated to it in AxylOS repository itself. For example, in your forked repository you should create the PKGBUILD file in another directory as `AxylOS/packages/pentesting/<yourdependency>/PKGBUILD`. The PKGBUILD related to a depedency of a tool does not need of the field `groups=('AxylOS' MORE_AxylOS_GROUPS_HERE).`
:::

## Updating a package

If you noticed that the repository is not keeping the latest version of a tool:
1. Access to its PKGBUILD in [AxylOS Repository](https://github.com/AxylOS-OS/AxylOS/tree/main/packages/pentesting) by visiting its directory by **https://github.com/AxylOS-OS/AxylOS/tree/main/packages/pentesting/toolname** and inside it there should be a PKGBUILD file.
