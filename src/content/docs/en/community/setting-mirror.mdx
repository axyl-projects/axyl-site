...
At the beginning, Arch-based AxylOS synchronized mirrors by git protocol but it was not suitable for package mirroring for the nature of git usage. One suitable option to consider has been rsync protocol.
...
The purpose of this guide is to set a mirror for AxylOS ISO images and AxylOS Arch packages. In AxylOS Nix there is no need to mirror packages because package mirror system is not used on Nix environment.
...
* CPU: >= 4 cores
* RAM: >= 8 GB
* Storage: the AxylOS repository size is around 5 GB, so you can set at least x3 repository size as server storage
* Bandwidth: >= 1 Gbps
...
```shell
sudo mkdir -p /srv/mirrors/AxylOS{,-images}
sudo chown -R rsyncuser:rsyncuser /srv/mirrors/AxylOS
sudo chown -R rsyncuser:rsyncuser /srv/mirrors/AxylOS-images
```
...
[AxylOS]
path = /srv/mirrors/AxylOS
comment = The AxylOS Hub
read only = true

[AxylOS-images]
path = /srv/mirrors/AxylOS-images
comment = The AxylOS ISO images
read only = true
```
...
{/*
Download the public key of `hub.AxylOSos.org` and set it among your authorized keys by:
```shell
su - rsyncuser
mkdir -p ~/.ssh/
chmod 0700 ~/.ssh/
wget -O - -q https://hub.AxylOSos.org/mirrorkey.pub >> ~/.ssh/authorized_keys
...
```
hub.AxylOSos.org will push and trigger delta changes to the new destination mirror server.
NOTE: currently we use PULL approach instead of PUSH approach, so it is not the main AxylOS mirror server that pushes changes to your new mirror, but it is the new mirror that pulls changes from main AxylOS mirror server.
*/}

{/*
To pull and synchronize AxylOS packages, you must create a SSH keypair and provide the public key to AxylOS Team. In particular, generate a SSH keypair:
```shell
su - rsyncuser
...
To keep your mirror up-to-date, you should regularly sync with the AxylOS repositories. First, enable `cron` service:
...
```
0 *\/1 * * * rsync -avzzlr --delete -e ssh rsyncuser@hub.AxylOSos.org:/srv/mirrors/ /srv/mirrors/
```
*/}
```
0 */1 * * * rsync -avzzlr --delete rsync://hub.AxylOSos.org/AxylOS /srv/mirrors/AxylOS
0 */1 * * * rsync -avzzlr --delete rsync://hub.AxylOSos.org/AxylOS-images /srv/mirrors/AxylOS-images
```
...
Create `/etc/nginx/sites-available/AxylOSmirror` with the following content:
```
server {
    listen 80;
...
```shell
sudo ln -s /etc/nginx/sites-available/AxylOSmirror /etc/nginx/sites-enabled/
```
...
Finally, on your AxylOS or Arch Linux environment, you can test it on your test AxylOS client by adding your mirror in `/etc/pacman.d/AxylOS-mirrorlist` by:
```
...
After obtaining the certificate, Certbot will automatically update your Nginx configuration to use HTTPS. If, for any reason, this doesn't happen, you can manually edit `/etc/nginx/sites-available/AxylOSmirror` by:
```
server {
    server_name your_mirror_domain.com;
...
| -------- | ---- | ----------- | ---------- |
| Rsync | 873/TCP | hub.AxylOSos.org | 89.116.236.99 |

### Publish

{/*
At this point the new mirror server cannot still retrieve information from the main AxylOS mirror. To do that, get the content of the previously created SSH public key by:
```shell
su -s rsyncuser
cat ~/.ssh/id_rsa.pub
```
*/}
Finally, to be deployed to all users, your mirror must be added communicated to AxylOS Team in order to be added in the official AxylOS mirrorlist.

Send the following information to **keeper@AxylOSos.org**:
* public hostname
* scheduled sync time
* email to inform you in case we make changes on mirroring design

Once AxylOS Team accepts your request, you will be informed your mirror has been added on AxylOS mirrorlist.
