---
title: GNOME
description: GNOME environment in AxylOS.
---
...
In AxylOS, GNOME has been configured in order to decrease the usage complexity and to improve the user desktop experience.
...
On the right side of the top bar, by clicking on the statement **AxylOS is Great!**, a set of widgets appears.
...
{/*
GNOME in AxylOS offers also the **Gestures Improvement** extension that allow you to improve touchpad gestures. It can be managed by accessing to the Extensions, finding this extension and clicking on **Settings**.
...
I you wish to change some GNOME elements in AxylOS, you can follow this guide to get the right steps and transform AxylOS according to your taste.
...
`/run/current-system/sw/share/backgrounds/AxylOS/` folder.
</TabItem>
</Tabs>
...
If you wish to apply one of the complete themes prebuilt for AxylOS:

<Tabs>
<TabItem label="AxylOS Arch">
the theme folder must always be stored in `/usr/share/themes/` or `~/.local/share/themes/`

If you wish to apply one of the complete themes prebuilt for AxylOS, you can use **AxylOS-theme-tweak** and using the **-t** parameter. Here the output of the help command to show how AxylOS Theme Tweak must be used:
```shell
List of arguments:
...
-l, --list                    list all available AxylOS themes
-t, --theme <theme-name>      specify an AxylOS theme to be set [AkameGaKill|BlueEyesSamurai|Graphite|CyborgGruvbox|SweetDark|XXE]

Usage Examples:
AxylOS-theme-tweak -l
AxylOS-theme-tweak -e alacritty
AxylOS-theme-tweak -t BlueEyesSamurai
AxylOS-theme-tweak -b
```
</TabItem>
<TabItem label="AxylOS Nix">
...
Here are reported some useful **keybindings** set by default in AxylOS GNOME.
...
<Tabs>
<TabItem label="AxylOS Arch">
To install AxylOS GNOME environment on side of an existing environment, run:
```
sudo pacman -S AxylOS-gnome-config
dconf load /org/gnome/settings-daemon/plugins/media-keys/ < /usr/share/AxylOS-gnome-config/dconf-custom-keybindings.ini
dconf load /org/gnome/mutter/ < /usr/share/AxylOS-gnome-config/dconf-mutter.ini
dconf load /org/gnome/desktop/ < /usr/share/AxylOS-gnome-config/dconf-desktop.ini
dconf load /org/gnome/shell/ < /usr/share/AxylOS-gnome-config/dconf-shell.ini
```
...
<TabItem label="AxylOS Nix">
To set AxylOS GNOME environment on side of an existing environment, just edit `/etc/nixos/configuration` and change `desktop` variable to `gnome`. Finally, run:
```shell
