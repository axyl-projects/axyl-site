---
title: AxylOS Nix Configuration
description: How to manage Nix configuration in AxylOS.
---
...
The macrostructure of AxylOS Nix configuration elements is the following:
```
[ 198]  /etc/nixos/
├── [  92]  home-manager/
│   ├── [  40]  browsers/
...
AxylOS Nix can be tailored to your needs. By `/etc/nixos/configuration.nix`, you can configure your entire system according to your need.
...
theme = "temple";
```
This variable is used to set the AxylOS theme on your environment. AxylOS offers several themes you can choose:
```
akame
anunna
cyborg
...
desktop = "gnome";
```
This variable is used to set your desktop environment. AxylOS Nix the current available environments are:
```
gnome
cinnamon
mate
...
dmanager = "gdm";
```
This variable is used to set your display manager. AxylOS Nix the current available display managers are:
```
gdm
lightdm neon
```
...
browser = "firefox";
```
This variable is used to set the browser application. Currently, only Firefox is supported in AxylOS Nix.
...
You can still deeply configure your AxylOS environment by acting on elements of its Nix macrostructure by adding or editing the related `default.nix` file.

For example, if you wish to edit the AxylOS Firefox configuration, just access to `/etc/nixos/home-manager/browsers/firefox/default.nix`.
...
They are just few examples. Familiarize with the AxylOS Nix configuration macrostructure to unlock the full potential of your environment!
...
* if it is caused by a generic software in AxylOS, you can access to `/etc/nixos/hosts/software/default.nix` and comment the line related to the tool that is broken;
