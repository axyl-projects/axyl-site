...
Purpose of this chapter is to test a GitHub workflow of AxylOS repository in a local machine.
...
Clone AxylOS repository:
```shell
git clone https://github.com/AxylOS-OS/AxylOS
cd AxylOS
```
...
Boot into AxylOS Live Environment by using AxylOS ISO.
...
nix-channel --update
```
Then, generate the hardware configuration and get the AxylOS Nix configuration:
```shell
mkdir -p /mnt/etc/nixos
nix-shell -p nixos-install-tools --command "nixos-generate-config --root /mnt"
curl -o /tmp/AxylOS-nix.zip https://codeload.github.com/AxylOS-OS/AxylOS-nix/zip/refs/heads/main
unzip /tmp/AxylOS-nix.zip -d /tmp/
cp -rf /tmp/AxylOS-nix-main/nixos/home-manager /tmp/AxylOS-nix-main/nixos/hosts /tmp/AxylOS-nix-main/nixos/modules /tmp/AxylOS-nix-main/nixos/pkgs /tmp/AxylOS-nix-main/nixos/users /tmp/AxylOS-nix-main/nixos/configuration.nix /tmp/AxylOS-nix-main/nixos/default.nix /mnt/etc/nixos/
...
```
Finally, run the AxylOS Nix installation:
```shell
