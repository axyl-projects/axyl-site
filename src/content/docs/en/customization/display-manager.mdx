...
LightDM in AxylOS uses **lightdm-webkit-theme-aether** theme package that is based on the **WebKit2 Greeter** that are the components that give you the current LightDM rice on AxylOS.
...
AxylOS uses SDDM Astronaut theme. If you like the dark style but you want to change the background, store your new background in `/usr/share/sddm/themes/astronaut/Assets/Wallpapers/` folder, then edit `Background=` variable in `/usr/share/sddm/themes/astronaut/theme.conf` and set your background image filename as:
```
# [General]
