...
description: How to install AxylOS as as guest in Hyper-V.
---
import ImageComponent from "@components/ImageComponent.astro";
import ThemedImage from '@components/ThemedImage.astro';

**Hyper-V** is a hypervisor that is included with some versions of Microsoft Windows. For using AxylOS as guest in Hyper-V, you need to [enable it](https://docs.microsoft.com/en-us/virtualization/hyper-v-on-windows/quick-start/enable-hyper-v).
...
Click on **Quick Create...** on the right pane. A window will popup. Click on **Location installation source** button and on **Change installation source...** button for loading AxylOS ISO image file, uncheck **This virtual machine will run Windows**, and on the bottom-right side, expand **More options** and type the name of the VM and click on **Create Virtual Machine**
...
The virtual machine will run with a 1024x768 resolution. For changing it, go to AxylOS settings -> Display and change it to your favourite resolution
...
Once booted, install AxylOS as explained in [Installing as single boot](/en/installation/install-single-boot/) chapter.
:::note
AxylOS automatically recognizes if you are using a virtualized environment so you don't need to install Hyper-V Guest Tools because the system will do automatically for you during the OS installation.
:::
<ImageComponent />
