...
description: How to install AxylOS as as guest in Quickemu.
---
...
Once Quickemu is installed, there are two simple steps to create and run AxylOS:
* **quickget** automatically downloads the ISO image for AxylOS and creates a configuration file for the virtual machine:
  ```shell
  quickget AxylOSos v23.11
  ```
* **quickemu** starts the virtual machine using the configuration file created by `quickget`:
  ```shell
  quickemu --vm AxylOSos-v23.11.conf
  ```
:::tip
