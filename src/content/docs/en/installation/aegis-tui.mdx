...
[Aegis TUI](https://github.com/AxylOS-OS/aegis-tui) is a terminal user interface frontend of **Aegis**, a tailored installer developed and optimized specifically for AxylOS.
...
Choose your favourite base system between AxylOS Arch or AxylOS Nix
...
:::tip
If you don't type any value for username or password, the default credential will be `AxylOS:AxylOS`.
:::
...
:::tip
If you don't type any value for hostname, the default value will be `AxylOSos`.
:::
...
Select the install parameters for the installation process (**AxylOS Nix only**)
...
config-logs
```
to get the links to https://termbin.com where the logs are temporary saved. Communicate these links to AxylOS Team for troubleshooting.

:::tip
If the screen locks due to user inactivity, unlock the screen by using `AxylOS` as password.
:::

If no errors have been raised during the installation phase, you are ready to use AxylOS!

![Installation Finished](@assets/images/aegis-tui-finish.png)

Before rebooting, if you installed AxylOS in dual boot with Windows, as explained in the related section, restore Microsoft bootloader inside the boot partition (i.e., `/dev/nvme0n1p1`):
```shell
...
For AxylOS Nix installation, once reboot and land on the installed system, initialize Nix Channels by opening the terminal and running:
```shell
